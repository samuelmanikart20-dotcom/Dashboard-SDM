...
export default function IKTDashboard() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data && result.data.length > 0) {
          const list = result.data as Daerah[];
          
          // Tambahkan opsi StandAlone di paling atas
          const allOption: Daerah = {
            id: 0,
            nama: "Konsolidasi IKT (Semua Branch)",
            kode: "StandAlone",
          };
          ...
          const withAll = [allOption, ...fallbackDaerah];
  ...
  const confirmDeleteData = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/ikt-data?bulan=${bulanInt}&tahun=${tahunInt}`, {
        method: 'DELETE',
      });
      ...
      if (data.success) {
        ...
        // Show success alert
        alert(`Berhasil menghapus ${data.deletedCount || 0} data IKT untuk bulan ${bulanName} ${tahunValue}`, 'success');
        ...
        alert(data.error || 'Gagal menghapus data IKT', 'error');
      }
    } catch (error) {
      console.error('Error deleting IKT data:', error);
      alert('Terjadi kesalahan saat menghapus data IKT', 'error');
    } finally {
      setDeleting(false);
    }
  };
  ...
  const pendidikanDistribution = (() => {
    ...
    const normalize = (val: string): keyof typeof buckets | null => {
      ...
      if (/(SMA|SMK|SMU|SLTA|SMP|SD)/.test(v)) return "SMA";
  ...
  return (
    ...
    <div className="bg-gradient-to-r text-black">
      ...
      <div className="px-3 lg:px-6 py-8 space-y-6">
        ...
        {/* Hapus Data IKT Section */}
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">
            Hapus Data IKT per Bulan
          </h2>
          <p className="text-gray-600 mb-6 text-center">
            Pilih bulan dan tahun untuk menghapus data IKT dari sistem. Tindakan ini tidak dapat dibatalkan.
          </p>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex justify-center">
              ...
              >
                {deleting ? 'Menghapus...' : 'Hapus Data IKT'}
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="text-center">
                ...
                <div className="mt-3 px-4">
                  <p className="text-sm text-gray-600">
                    Apakah Anda yakin ingin <span className="font-medium text-red-600">menghapus</span> semua data IKT untuk bulan <span className="font-medium">{getBulanName()}</span> <span className="font-medium">{deleteTahun}</span>?<br />Tindakan ini tidak dapat dibatalkan.
                  </p>
                </div>
