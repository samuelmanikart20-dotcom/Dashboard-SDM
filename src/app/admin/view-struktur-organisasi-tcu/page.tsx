...
export default function ViewStrukturOrganisasiTCUPage() {
  const [selectedDirektorat, setSelectedDirektorat] = useState<string>("all"); // Default to "all" for TCU
  ...
  const fetchAvailablePeriodsAndDaerah = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/tcu-struktur-organisasi/available-periods", {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error(`[ViewStrukturOrganisasi TCU] API error: ${response.status} - ${errorText}`);
      ...
      if (!result.success) {
        console.error(`[ViewStrukturOrganisasi TCU] API returned error:`, result.error);
      ...
      if (result.data) {
        ...
        console.log(`[ViewStrukturOrganisasi TCU] Received ${periods.length} periods and ${directorates.length} directorates from API`);
        ...
        if (periods.length > 0) {
          ...
          if (!selectedPeriod || selectedPeriod !== latestPeriod) {
            ...
            console.log(`[ViewStrukturOrganisasi TCU] Set default period to: ${latestPeriod}`);
          }
        }
        
        // Default selectedDirektorat is already "all"
        
        console.log(`[ViewStrukturOrganisasi TCU] Loaded ${periods.length} periods and ${directorates.length} directorates`);
      } else {
        console.warn(`[ViewStrukturOrganisasi TCU] No data in response`);
        setAvailablePeriods([]);
        setDirektoratList([]);
      }
      } catch (error: any) {
        ...
        console.error(`[ViewStrukturOrganisasi TCU] Error details:`, errorMessage);
        setAvailablePeriods([]);
        setDirektoratList([]);
      } finally {
        setLoading(false);
  ...
  const handlePrint = () => {
    ...
    if (selectedDirektorat === "all") {
          ...
          orgChartsHTML += `
            <div class="print-direktorat-section" style="page-break-after: always; margin-bottom: 40px;">
              <div class="print-header">
                <h1>STRUKTUR ORGANISASI TCU - ${direktorat.label}</h1>
      ...
      orgChartsHTML = `
        <div class="print-header">
          <h1>STRUKTUR ORGANISASI TCU - ${direktoratLabel}</h1>
          <p>Periode: ${periodLabel}</p>
        </div>
        <div class="print-container">
    ...
    const printHTML = `
      ...
      <html>
        <head>
          <title>Struktur Organisasi TCU - ${printTitle}</title>
          <style>
            ${styles}
            @page {
              size: A3 landscape;
