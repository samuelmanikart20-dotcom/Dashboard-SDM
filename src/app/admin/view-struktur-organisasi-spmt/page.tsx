...
export default function ViewStrukturOrganisasiSPMTPage() {
  ...
  const fetchAvailablePeriodsAndDaerah = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/struktur-organisasi/available-periods", {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error(`[ViewStrukturOrganisasi] API error: ${response.status} - ${errorText}`);
      ...
      if (!result.success) {
        console.error(`[ViewStrukturOrganisasi] API returned error:`, result.error);
      ...
      if (result.data) {
        ...
        console.log(`[ViewStrukturOrganisasi SPMT] Received ${periods.length} periods and ${directorates.length} directorates from API`);
        console.log(`[ViewStrukturOrganisasi SPMT] Directorate list:`, directorates);
        ...
        if (periods.length > 0) {
          ...
          if (!selectedPeriod || selectedPeriod !== latestPeriod) {
            ...
            console.log(`[ViewStrukturOrganisasi SPMT] Set default period to: ${latestPeriod}`);
          }
        }
        
        // Default selectedDirektorat is already "all" (like IKT)
        
        console.log(`[ViewStrukturOrganisasi SPMT] Loaded ${periods.length} periods and ${directorates.length} directorates`);
      } else {
        console.warn(`[ViewStrukturOrganisasi SPMT] No data in response`);
        setAvailablePeriods([]);
        setDirektoratList([]);
      }
    } catch (error: any) {
      ...
      console.error(`[ViewStrukturOrganisasi] Error details:`, errorMessage);
        ...
        {selectedPeriod ? (
          selectedDirektorat === "all" ? (
            ...
            <div className="flex-1 flex flex-col h-[calc(100vh-100px)]">
              <div className="bg-white px-4 py-2 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800">
                  Struktur Organisasi SPMT - {direktoratList.find(d => d.value === selectedDirektorat)?.label || ""}
                </h2>
                <p className="text-sm text-gray-600">
                  Periode: {availablePeriods.find(p => p.value === selectedPeriod)?.label || selectedPeriod}
                </p>
