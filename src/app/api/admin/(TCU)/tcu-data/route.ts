...
// DELETE - Delete TCU data by bulan and tahun
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const [existingRecords] = await connection.execute(
      'SELECT COUNT(*) as count FROM tcudata WHERE bulan = ? AND tahun = ?',
      [bulanInt, tahunInt]
    );
    ...
    return NextResponse.json({
      success: true,
      message: `Berhasil menghapus ${affectedRows} data TCU untuk bulan ${bulanInt} tahun ${tahunInt}`,
    ...
    return NextResponse.json(
      { success: false, error: 'Gagal menghapus data TCU' },
      { status: 500 }
    );
  } finally {
    if (connection) {
