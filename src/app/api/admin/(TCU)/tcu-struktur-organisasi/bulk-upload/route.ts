...
function formatDateForMySQL(dateValue: any): string | null {
  ...
  if (typeof dateValue === 'string') {
    ...
    if (wrongFormatMatch) {
      ...
      // Jika gagal konversi, lanjutkan ke parsing normal (jangan return null langsung)
    }
...
// Ensure direktorat exists in daerah table (untuk TCU)
async function ensureDirektoratDaerah(
  ...
  if (Array.isArray(existing) && existing.length > 0) {
    ...
    daerahNameMap.set(namaUpper.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim(), id);
    daerahNameMap.set(namaNormalized.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim(), id);
    return id;
  }
  ...
  daerahNameMap.set(namaUpper.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim(), newId);
  daerahNameMap.set(namaNormalized.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim(), newId);
  return newId;
}
...
// Deteksi kolom DIREKTORAT dengan variasi nama
function detectDirektoratColumn(headers: string[]): string | null {
  const direktoratVariants = ['DIREKTORAT', 'DIREKTORAT_ID', 'KODE DIREKTORAT', 'NAMA DIREKTORAT', 'KODE_DIREKTORAT', 'NAMA_DIREKTORAT', 'DEPT', 'DIVISION'];
  for (const variant of direktoratVariants) {
...
function findDaerahId(
  ...
  // 2. Direct match dengan nama lengkap (dengan prefix SPMT/PTP/TCU)
  ...
  // 4. Match nama tanpa prefix "SPMT", "PTP", atau "TCU" (contoh: "Belawan" -> "SPMT Belawan")
  const namaWithoutPrefix = daerahUpper.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim();
  if (namaWithoutPrefix.length > 0) {
    for (const row of daerahRows) {
      const dbNama = normalize(String(row.nama || ''));
      const dbNamaWithoutPrefix = dbNama.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim();
  ...
  const keywords = daerahUpper.split(/\s+/).filter(k => k.length > 2 && k !== 'SPMT' && k !== 'PTP' && k !== 'TCU');
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const workbook = read(buffer, { 
      cellDates: false, // Jangan konversi otomatis ke Date object, kita handle sendiri
      cellNF: false,
      cellText: false 
    });
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    // Gunakan raw: true agar Excel date serial numbers tetap sebagai number
    // Ini memungkinkan formatDateForMySQL untuk mendeteksi dan mengkonversi dengan benar
    const data = utils.sheet_to_json<any>(sheet, { 
      defval: null,
      raw: true // Pertahankan tipe data asli (number untuk Excel serial dates)
    });
    ...
    // PERBAIKAN: Untuk TCU, gunakan kolom DIREKTORAT untuk mapping ke tabel daerah (seperti PTP)
    // Prioritas: DIREKTORAT dulu, baru fallback ke DAERAH
    ...
    // Prioritas 1: Gunakan DIREKTORAT jika ada (untuk TCU)
    if (direktoratColumn) {
      daerahColumnName = direktoratColumn;
      usingDirektoratFallback = true;
      console.log('[TCU Bulk Upload] Using DIREKTORAT column for mapping to daerah.');
      ...
      console.log('[TCU Bulk Upload] DIREKTORAT column not found. Using DAERAH column.');
    } else {
      // Error jika kedua kolom tidak ditemukan
      return NextResponse.json(
        { success: false, error: 'Kolom DIREKTORAT atau DAERAH tidak ditemukan. Untuk TCU, pastikan file Excel memiliki kolom "DIREKTORAT" untuk mapping ke daerah.' },
        { status: 400 }
      );
    }
    ...
    // Get mapping daerah dari database (daerah) - ORDER BY untuk konsistensi
    ...
    // Log untuk debug
    console.log(`[DEBUG TCU] Found ${(daerahRows as any[]).length} daerah in database`);
    if ((daerahRows as any[]).length > 0) {
      console.log(`[DEBUG TCU] All daerah:`, (daerahRows as any[]).map(r => ({ id: r.id, nama: r.nama, kode: r.kode })));
    }
    ...
    for (const row of daerahRows as any[]) {
      ...
      if (nama) {
        ...
        daerahNameMap.set(namaUpper.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim(), row.id); // Without prefix
        daerahNameMap.set(namaNormalized.replace(/^(SPMT|PTP|TCU)\s+/i, '').trim(), row.id); // Without prefix, normalized
      }
    }
    
    console.log(`[DEBUG TCU] daerahMap size: ${daerahMap.size}, daerahNameMap size: ${daerahNameMap.size}`);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!daerahId && usingDirektoratFallback) {
        ...
        if (daerahId) {
          console.log(`[TCU Bulk Upload] Created new direktorat entry "${daerahValue}" with daerah_id ${daerahId}`);
        }
      }
      ...
      if (!daerahId) {
        ...
        errors.push({
          ...
          daerah: daerahValue,
          suggestion: `Format yang didukung: "SPMT Belawan", "Belawan", atau "BLW" (kode). Contoh daerah yang ada: ${availableDaerah}...`
        });
        continue;
      }
        ...
        for (const [periodeKey, periodeRows] of rowsByPeriode.entries()) {
            ...
            try {
              ...
              for (const row of existingRows as any[]) {
                if (row.photo_url && row.photo_url.trim() !== '') {
                  ...
                  console.log(`[tcu-bulk-upload] Preserving photo for ${row.id_posisi_sap}: ${row.photo_url}`);
                }
              }
              
              if (existingPhotos.size > 0) {
                console.log(`[tcu-bulk-upload] Found ${existingPhotos.size} existing photos to preserve for periode ${bulanPeriode}/${tahunPeriode}`);
              }
            } catch (err) {
              console.error(`[tcu-bulk-upload] Error fetching existing photos:`, err);
              // Continue dengan DELETE meskipun error
            }
            
            // Baru hapus data lama yang akan di-replace
            await conn.execute(
              `DELETE FROM struktur_organisasi_tcu 
               WHERE daerah_id = ? AND bulan = ? AND tahun = ? 
               AND id_posisi_sap IN (${placeholders})`,
              [daerahId, bulanPeriode, tahunPeriode, ...sapIdsArray]
            );
          ...
          for (const row of periodeRows) {
            try {
              ...
              const direktorat = normalizeValue(getFieldFromRow(row, ["DIREKTORAT", "DIRECTORATE", "DIVISI", "DIVISION", "direktorat", "directorate"]));
              ...
              const noHp = normalizeValue(getFieldFromRow(row, ["NO HP", "NO. HP", "NO_HP", "no_hp", "No. HP", "no hp", "No HP", "HANDHPONE", "handphone", "HP", "hp", "NOMOR TELEPON", "NOMOR TELEPON/KONTAK", "Nomor Telepon/Kontak"]));
              
              // PERBAIKAN: Untuk tanggal, ambil nilai langsung (bisa berupa Excel serial number atau string)
              // Jangan normalize dulu karena bisa menghilangkan informasi Excel date serial number
              const tmtJabatanRaw = getFieldFromRow(row, [
                "TMT JABATAN", "TMT_JABATAN", "tmt_jabatan", "Tmt Jabatan", "tmt jabatan", "TMT",
                "TANGGAL EFEKTIF", "TANGGAL EFEKTIF/PERIODE JABATAN", "Tanggal Efektif/Periode Jabatan",
                "TMT", "tmt", "TANGGAL", "tanggal"
              ]);
              ...
              if (!finalPhotoUrl) {
                ...
                if (existingPhoto) {
                  ...
                  console.log(`[tcu-bulk-upload] Using preserved photo for ${sap}: ${existingPhoto}`);
                } else {
                  ...
                  if (prevPhoto) {
                    ...
                    daerahFotoDariPeriodeSebelumnya++;
                    console.log(`[tcu-bulk-upload] Using photo from previous period for ${sap}: ${prevPhoto}`);
                  }
                }
              }
