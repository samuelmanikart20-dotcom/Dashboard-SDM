...
function formatDateForMySQL(dateValue: any): string | null {
  ...
  try {
    ...
    if (
      ...
      (typeof dateValue === "string" && /^\d+$/.test(String(dateValue).trim()))
    ) {
      const numValue =
        typeof dateValue === "number"
          ? dateValue
          : parseFloat(String(dateValue).trim());
      ...
      if (numValue >= 1 && numValue <= 100000 && Number.isInteger(numValue)) {
        ...
        if (year >= 1900 && year <= 2100) {
          ...
          console.log(
            `[formatDateForMySQL] Converted Excel serial number ${numValue} to ${formatted}`
          );
          return formatted;
        } else {
          console.warn(
            `[formatDateForMySQL] Excel serial number ${numValue} resulted in invalid year ${year}, returning null`
          );
          return null;
        }
      }
        ...
        if (wrongFormatMatch) {
          ...
          if (serialNumber >= 1 && serialNumber <= 100000) {
            ...
            if (year >= 1900 && year <= 2100) {
              ...
              const formatted = `${year}-${month}-${day}`;
              console.log(
                `[formatDateForMySQL] Converted wrong format "${strValue}" (serial ${serialNumber}) to ${formatted}`
              );
              return formatted;
            }
          }
    ...
    if (year < 1900 || year > 2100) {
      console.warn(
        `[formatDateForMySQL] Date ${dateValue} resulted in invalid year ${year}, returning null`
      );
      return null;
    }
...
async function ensureTable(conn: mysql.Connection) {
  ...
  const ddl = `
    CREATE TABLE IF NOT EXISTS struktur_organisasi_tcu (
      ...
      UNIQUE KEY uniq_daerah_id_sap_periode_tcu (daerah_id, id_posisi_sap, bulan, tahun),
      KEY idx_daerah_tcu (daerah_id),
      KEY idx_periode_tcu (bulan, tahun),
  ...
  const [cols]: any = await conn.execute(
    `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'struktur_organisasi_tcu'`,
    [dbConfig.database]
  );
  ...
  if (!have.has("periode_jabatan"))
    alters.push(`ADD COLUMN periode_jabatan VARCHAR(128) NULL`);
  if (!have.has("kj_individu"))
    alters.push(`ADD COLUMN kj_individu VARCHAR(64) NULL`);
  if (!have.has("kj_posisi"))
    alters.push(`ADD COLUMN kj_posisi VARCHAR(64) NULL`);
  if (!have.has("bulan")) alters.push(`ADD COLUMN bulan INT NULL`);
  if (!have.has("tahun")) alters.push(`ADD COLUMN tahun INT NULL`);

  if (alters.length) {
    await conn.execute(
      `ALTER TABLE struktur_organisasi_tcu ${alters.join(", ")}`
    );
  }
  ...
  if (!have.has("bulan") || !have.has("tahun")) {
    try {
      await conn.execute(
        `ALTER TABLE struktur_organisasi_tcu DROP INDEX IF EXISTS uniq_daerah_id_sap_tcu`
      );
      await conn.execute(
        `ALTER TABLE struktur_organisasi_tcu ADD UNIQUE KEY uniq_daerah_id_sap_periode_tcu (daerah_id, id_posisi_sap, bulan, tahun)`
      );
      await conn.execute(
        `ALTER TABLE struktur_organisasi_tcu ADD INDEX idx_periode_tcu (bulan, tahun)`
      );
    } catch (e: any) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const useDirectorateFilter =
      !!directorateParam &&
      directorateParam.trim() !== "" &&
      directorateParam !== "all";
    const directorateValue = useDirectorateFilter
      ? directorateParam!.trim()
      : null;
    ...
    // PERBAIKAN: Untuk TCU, ambil semua data untuk periode yang sama (tidak hanya satu daerah)
    ...
    if (month && year && month !== "all" && year !== "all") {
      ...
      try {
        let latestPeriodQuery = `SELECT bulan, tahun FROM struktur_organisasi_tcu 
             WHERE bulan IS NOT NULL AND tahun IS NOT NULL`;
        const latestParams: any[] = [];
        // Untuk TCU, ambil periode terbaru dari semua daerah (tidak filter per daerah)
        ...
        if (Array.isArray(latestPeriod) && latestPeriod.length > 0) {
          ...
          const daerahLabel = isAllDaerah
            ? "Semua Daerah"
            : `daerah_id=${daerahId}`;
          const directorateLabel = useDirectorateFilter
            ? `, direktorat=${directorateValue}`
            : "";
          console.log(
            `[tcu-struktur-organisasi/positions] Using latest period: bulan=${latest.bulan}, tahun=${latest.tahun} for ${daerahLabel}${directorateLabel}`
          );
        } else {
          const daerahLabel = isAllDaerah
            ? "Semua Daerah"
          ...
          const directorateLabel = useDirectorateFilter
            ...
            : "";
          console.log(
            `[tcu-struktur-organisasi/positions] No period data found for ${daerahLabel}${directorateLabel}, fetching all data`
          );
        }
      } catch (e) {
    ...
    const [rows] = await conn.execute(
      `SELECT 
        ...
        COALESCE(
          s.photo_url,
          (SELECT photo_url 
           FROM struktur_organisasi_tcu s2
        ...
        s.bulan, 
        s.tahun
       FROM struktur_organisasi_tcu s ${whereClause} ORDER BY s.id_posisi_sap ASC`,
      params
    );

    // PERBAIKAN: Untuk TCU, jika ada ID POSISI SAP yang sama di beberapa daerah,
    ...
    // PERBAIKAN: Untuk TCU, buat ID node yang unik untuk setiap kombinasi ID POSISI SAP + daerah_id
    ...
    if (targetDaerahId) {
      ...
      // Untuk setiap required parent ID, cari data dengan prioritas:
      // 1. Dari daerah target DAN sistem (unit_kerja) yang sama
      // 2. Dari daerah target (tanpa filter sistem)
      // 3. Dari sistem yang sama (tanpa filter daerah)
      // 4. Dari manapun
      const parentData = new Map<string, any>();
      for (const parentId of requiredParentIds) {
        // Cari child yang membutuhkan parent ini untuk mengetahui sistemnya
        const childNeedingParent = dataFromTargetDaerah.find(
          (m) => m.id_posisi_atasan && String(m.id_posisi_atasan).trim() === parentId
        );
        const childUnitKerja = childNeedingParent?.unit_kerja 
          ? String(childNeedingParent.unit_kerja).trim() 
          : null;
        
        let parentRow = null;
        
        // Prioritas 1: Dari daerah target DAN sistem yang sama
        if (childUnitKerja) {
          parentRow = dataFromTargetDaerah.find(
            (m) => 
              m.id_posisi_sap === parentId &&
              m.unit_kerja &&
              String(m.unit_kerja).trim() === childUnitKerja
          );
        }
        
        // Prioritas 2: Dari daerah target (tanpa filter sistem)
        if (!parentRow) {
          parentRow = dataFromTargetDaerah.find(
        ...
        // Prioritas 3: Dari sistem yang sama (tanpa filter daerah)
        if (!parentRow && childUnitKerja) {
          parentRow = allMapped.find(
            (m) => 
              m.id_posisi_sap === parentId &&
              m.unit_kerja &&
              String(m.unit_kerja).trim() === childUnitKerja
      ...
      for (const parentRow of parentData.values()) {
        ...
        if (
          !result.some(
            (r) =>
              r.id_posisi_sap === parentRow.id_posisi_sap &&
    ...
    console.error(
      "GET /api/admin/tcu-struktur-organisasi/positions error",
      error
    );
    ...
    return NextResponse.json(
      {
        success: false,
        error: `Error loading TCU structure: ${errorMessage}`,
      },
      { status: 500 }
    );
  } finally {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!daerah_id || !Array.isArray(items)) {
      return NextResponse.json(
        { success: false, error: "daerah_id and items[] are required" },
        { status: 400 }
      );
    }
    ...
    if (!bulan || !tahun || bulan === "all" || tahun === "all") {
      return NextResponse.json(
        { success: false, error: "bulan and tahun are required" },
        { status: 400 }
      );
    }
    ...
    // Validasi: Pastikan daerah_id ada (gunakan tabel daerah untuk TCU)
    // Cek apakah daerah_id valid (bisa berupa string atau number)
    const daerahIdNum =
      typeof daerah_id === "string" ? parseInt(daerah_id) : daerah_id;
    ...
    if (!Array.isArray(daerahCheck) || daerahCheck.length === 0) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `daerah_id ${daerahIdNum} tidak ditemukan di tabel daerah. Pastikan daerah sudah ada di sistem.`,
        },
        { status: 400 }
      );
    }
    ...
    for (const it of items) {
      ...
      const finalBulan =
        it.bulan !== undefined && it.bulan !== null
          ? parseInt(String(it.bulan))
          : bulanInt;
      const finalTahun =
        it.tahun !== undefined && it.tahun !== null
          ? parseInt(String(it.tahun))
          : tahunInt;
    ...
    if (values.length === 0) {
      await conn.rollback();
      return NextResponse.json(
        { success: false, error: "No valid items to import" },
        { status: 400 }
      );
    }
    ...
    if (isBulkUpload) {
      ...
      console.log(
        `[tcu-struktur-organisasi/positions] Bulk upload: Deleting old records for daerah ${daerah_id}, periode ${bulanInt}/${tahunInt}`
      );
      await conn.execute(
        "DELETE FROM struktur_organisasi_tcu WHERE daerah_id = ? AND bulan = ? AND tahun = ?",
      ...
      console.log(
        `[tcu-struktur-organisasi/positions] Single node update: Using explicit UPDATE with WHERE clause for periode ${bulanInt}/${tahunInt}`
      );
      const singleValue = values[0];
      const [updateResult]: any = await conn.execute(
        `
        UPDATE struktur_organisasi_tcu
        SET
          id_posisi_atasan = ?,
          nama = ?,
          jabatan = ?,
      ...
      if (updateResult.affectedRows === 0) {
        console.log(
          `[tcu-struktur-organisasi/positions] No row updated, inserting new row for periode ${bulanInt}/${tahunInt}`
        );
        await conn.execute(
          `
          INSERT INTO struktur_organisasi_tcu
        ...
        console.log(
          `[tcu-struktur-organisasi/positions] Updated ${updateResult.affectedRows} row(s) for periode ${bulanInt}/${tahunInt}`
        );
      }
    }
    ...
    console.error(
      "POST /api/admin/tcu-struktur-organisasi/positions error",
      error
    );
    try {
