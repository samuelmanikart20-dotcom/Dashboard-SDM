...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Expected column mapping for IKT data (UPDATED to include PENDIDIKAN)
    // NPP, NAMA, TANGGAL LAHIR, NAMA JABATAN, ENTITAS, UNIT KERJA (KANTOR PUSAT/BRANCH),
    ...
    for (let i = 1; i < jsonData.length; i++) {
      ...
      try {
        ...
        const processedRow = {
          ...
          jenis_kelamin: row[7]?.toString().trim() || "",
          pendidikan: row[8]?.toString().trim() || "", // NEW
          organik_non_organik: row[9]?.toString().trim() || "",
          pusat_pelayanan: row[10]?.toString().trim() || "",
          non_operasional: row[11]?.toString().trim() || "",
    ...
    // Ensure iktdata table exists and has pendidikan column
    ...
    // Safe alter if legacy table missing pendidikan
    const [colRows] = await connection.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) as cnt FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'iktdata' AND COLUMN_NAME = 'pendidikan'`
    );
    ...
    try {
      ...
      const deleteResult = await connection.execute(
        "DELETE FROM iktdata WHERE bulan = ? AND tahun = ?",
        [bulan, tahun]
      );
      ...
      let message = `${insertedCount} data IKT berhasil diupload.`;
      if (deletedCount > 0) {
        message = `Replaced ${deletedCount} existing records. ${insertedCount} new IKT records inserted.`;
      }
