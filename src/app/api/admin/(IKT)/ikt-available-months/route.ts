  ...
  try {
    ...
    const monthlyPeriods = (rows as any[]).map(row => {
      ...
      if (isNaN(bulan) || bulan < 1 || bulan > 12) {
        console.warn(`IKT Available Months - Invalid bulan value: ${row.bulan}, skipping`);
        return null;
      }
    ...
    console.error('IKT Available months error:', error);
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to fetch IKT available months',
