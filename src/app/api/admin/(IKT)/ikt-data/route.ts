...
// DELETE - Delete IKT data by bulan and tahun
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const [existingRecords] = await connection.execute(
      'SELECT COUNT(*) as count FROM iktdata WHERE bulan = ? AND tahun = ?',
      [bulanInt, tahunInt]
    );
    ...
    return NextResponse.json({
      success: true,
      message: `Berhasil menghapus ${affectedRows} data IKT untuk bulan ${bulanInt} tahun ${tahunInt}`,
    ...
    return NextResponse.json(
      { success: false, error: 'Gagal menghapus data IKT' },
      { status: 500 }
    );
  } finally {
    if (connection) {
