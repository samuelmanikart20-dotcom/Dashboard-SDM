...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Cache daerah kode->id (SPMT uses general daerah table)
      const [daerahRows] = await conn.execute('SELECT id, kode FROM daerah')
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!daerahId) {
          throw new Error(`Baris ${i + 2}: Daerah tidak ditemukan untuk kode '${r.daerah_kode || r.daerah_id}'`)
        }
