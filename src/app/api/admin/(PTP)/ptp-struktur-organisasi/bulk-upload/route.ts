...
async function ensureDirektoratDaerah(
  ...
  const [existing]: any = await conn.execute(
    'SELECT id, kode FROM ptp_daerah WHERE UPPER(nama) = ? LIMIT 1',
    [upperName]
  );
  if (Array.isArray(existing) && existing.length > 0) {
    ...
    daerahNameMap.set(namaUpper.replace(/^(SPMT|PTP)\s+/i, '').trim(), id);
    daerahNameMap.set(namaNormalized.replace(/^(SPMT|PTP)\s+/i, '').trim(), id);
    return id;
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // PERBAIKAN: Prioritas kolom DIREKTORAT untuk PTP (bukan DAERAH)
    // Untuk PTP, kolom DIREKTORAT digunakan untuk mapping ke ptp_daerah
    ...
    // Prioritas 1: Gunakan DIREKTORAT jika ada (untuk PTP)
    if (direktoratColumn) {
      daerahColumnName = direktoratColumn;
      usingDirektoratFallback = true;
      console.log('[PTP Bulk Upload] Using DIREKTORAT column for mapping to ptp_daerah.');
      ...
      console.log('[PTP Bulk Upload] DIREKTORAT column not found. Using DAERAH column.');
    } else {
      // Error jika kedua kolom tidak ditemukan
      return NextResponse.json(
        { success: false, error: 'Kolom DIREKTORAT atau DAERAH tidak ditemukan. Untuk PTP, pastikan file Excel memiliki kolom "DIREKTORAT" untuk mapping ke ptp_daerah.' },
        { status: 400 }
      );
    }
    ...
    // Get mapping daerah dari database (ptp_daerah) - ORDER BY untuk konsistensi
    const [daerahRows]: any = await conn.execute('SELECT id, nama, kode FROM ptp_daerah ORDER BY id');
    ...
    // Log untuk debug
    console.log(`[DEBUG PTP] Found ${(daerahRows as any[]).length} daerah in database`);
    if ((daerahRows as any[]).length > 0) {
      console.log(`[DEBUG PTP] All daerah:`, (daerahRows as any[]).map(r => ({ id: r.id, nama: r.nama, kode: r.kode })));
    }
    ...
    for (const row of daerahRows as any[]) {
      ...
      if (nama) {
        ...
        daerahNameMap.set(namaUpper.replace(/^(SPMT|PTP)\s+/i, '').trim(), row.id); // Without prefix
        daerahNameMap.set(namaNormalized.replace(/^(SPMT|PTP)\s+/i, '').trim(), row.id); // Without prefix, normalized
      }
    }
    
    console.log(`[DEBUG PTP] daerahMap size: ${daerahMap.size}, daerahNameMap size: ${daerahNameMap.size}`);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!daerahId && usingDirektoratFallback) {
        ...
        if (daerahId) {
          console.log(`[PTP Bulk Upload] Created new direktorat entry "${daerahValue}" with daerah_id ${daerahId}`);
        }
      }
    ...
    for (const [daerahId, rows] of rowsByDaerah.entries()) {
      try {
        ...
        // Get daerah name
        const [daerahInfo]: any = await conn.execute('SELECT nama FROM ptp_daerah WHERE id = ?', [daerahId]);
            ...
            try {
              ...
              for (const row of existingRows as any[]) {
                if (row.photo_url && row.photo_url.trim() !== '') {
                  ...
                  console.log(`[ptp-bulk-upload] Preserving photo for ${row.id_posisi_sap}: ${row.photo_url}`);
                }
              }
              
              if (existingPhotos.size > 0) {
                console.log(`[ptp-bulk-upload] Found ${existingPhotos.size} existing photos to preserve for periode ${bulanPeriode}/${tahunPeriode}`);
              }
            } catch (err) {
              console.error(`[ptp-bulk-upload] Error fetching existing photos:`, err);
            ...
            try {
              ...
              if (!direktorat) {
                ...
                if (unitKerjaStr && unitKerjaStr !== '') {
                  ...
                  console.log(`[ptp-bulk-upload] Row ${row._row_number}: direktorat kosong, menggunakan unit_kerja: ${direktorat}`);
                }
              }
              ...
              if (!direktorat && usingDirektoratFallback && daerahColumnName) {
                ...
                if (mappingValue) {
                  ...
                  console.log(`[ptp-bulk-upload] Row ${row._row_number}: direktorat kosong, menggunakan mapping column (${daerahColumnName}): ${direktorat}`);
                }
              }
              
              // Log jika direktorat masih kosong untuk debugging (hanya beberapa row pertama)
              if (!direktorat && row._row_number <= 3) {
                console.log(`[ptp-bulk-upload] Row ${row._row_number}: direktorat is still null/empty after all attempts.`);
                console.log(`[ptp-bulk-upload] Row ${row._row_number}: unit_kerja:`, unitKerja);
                console.log(`[ptp-bulk-upload] Row ${row._row_number}: mappingColumn:`, daerahColumnName);
              }
              ...
              if (!finalPhotoUrl) {
                ...
                if (existingPhoto) {
                  ...
                  console.log(`[ptp-bulk-upload] Using preserved photo for ${sap}: ${existingPhoto}`);
                } else {
                  ...
                  if (prevPhoto) {
                    ...
                    daerahFotoDariPeriodeSebelumnya++;
                    console.log(`[ptp-bulk-upload] Using photo from previous period for ${sap}: ${prevPhoto}`);
                  }
                }
              }
