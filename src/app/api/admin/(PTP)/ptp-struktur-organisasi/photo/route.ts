...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(`[ptp-struktur-organisasi/photo] File berhasil disimpan: ${absPath} (${stats.size} bytes), skip_db_update=${skipDbUpdate}`);
    } catch (err) {
      console.error(`[ptp-struktur-organisasi/photo] Error memverifikasi file: ${absPath}`, err);
    ...
    if (skipDbUpdate) {
      console.log(`[ptp-struktur-organisasi/photo] skip_db_update=true, returning photo_url without database update`);
    ...
    if (bulan && tahun) {
      ...
      if (!isNaN(bulanInt) && !isNaN(tahunInt)) {
        console.log(`[ptp-struktur-organisasi/photo] Updating photo for daerah_id=${daerahId}, id_posisi_sap=${idPosisiSap}, periode=${bulanInt}/${tahunInt}`);
        
        // Cek apakah ada record untuk periode tersebut
        const [checkRows]: any = await conn.execute(
          `SELECT id, bulan, tahun, photo_url FROM struktur_organisasi_ptp WHERE daerah_id = ? AND id_posisi_sap = ? AND bulan = ? AND tahun = ?`,
          [daerahId, idPosisiSap, bulanInt, tahunInt]
        );
        
        console.log(`[ptp-struktur-organisasi/photo] Found ${(checkRows as any[]).length} records for periode ${bulanInt}/${tahunInt}`);
        
        if ((checkRows as any[]).length > 0) {
          // Update record untuk periode spesifik
          const [updateResult]: any = await conn.execute(
            `UPDATE struktur_organisasi_ptp SET photo_url = ? WHERE daerah_id = ? AND id_posisi_sap = ? AND bulan = ? AND tahun = ?`,
            [publicUrl, daerahId, idPosisiSap, bulanInt, tahunInt]
          );
          console.log(`[ptp-struktur-organisasi/photo] Updated ${updateResult.affectedRows} record(s) for periode ${bulanInt}/${tahunInt}`);
        } else {
          ...
          console.log(`[ptp-struktur-organisasi/photo] No record found for periode ${bulanInt}/${tahunInt}, creating new record from previous period`);
          
          // Cari data dari periode sebelumnya (periode terdekat yang lebih lama)
          const [prevRows]: any = await conn.execute(
            `SELECT * FROM struktur_organisasi_ptp 
          ...
          if (Array.isArray(prevRows) && prevRows.length > 0) {
            ...
            console.log(`[ptp-struktur-organisasi/photo] Created new record from previous period data with new photo_url`);
          } else {
            // Jika tidak ada data periode sebelumnya, update semua periode sebagai fallback
            console.log(`[ptp-struktur-organisasi/photo] No previous period data found, updating all periods as fallback`);
            await conn.execute(
              `UPDATE struktur_organisasi_ptp SET photo_url = ? WHERE daerah_id = ? AND id_posisi_sap = ?`,
        ...
        console.log(`[ptp-struktur-organisasi/photo] Invalid bulan/tahun, updating all periods`);
        await conn.execute(
          `UPDATE struktur_organisasi_ptp SET photo_url = ? WHERE daerah_id = ? AND id_posisi_sap = ?`,
          [publicUrl, daerahId, idPosisiSap]
        );
      ...
      console.log(`[ptp-struktur-organisasi/photo] No bulan/tahun provided, updating all periods`);
      await conn.execute(
        `UPDATE struktur_organisasi_ptp SET photo_url = ? WHERE daerah_id = ? AND id_posisi_sap = ?`,
        [publicUrl, daerahId, idPosisiSap]
      );
    ...
    console.error('POST /api/admin/ptp-struktur-organisasi/photo error', error);
