...
export default function TCUDashboard() {
  ...
  const fetchDaerahList = async () => {
    try {
      ...
      setDaerahList(dummyDaerah);
      setSelectedDaerah(dummyDaerah[0].id.toString());
    } catch (error) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const unitKerja = selectedDaerahInfo?.kode || "all";

      // Fetch real dashboard stats from API
      const statsResponse = await fetch(
        `/api/admin/tcu-dashboard-stats?bulan=${bulan}&tahun=${tahun}&unit_kerja=${unitKerja}`
      );
      if (!statsResponse.ok) {
      ...
      const tableResponse = await fetch(
        `/api/admin/tcu-table-data?bulan=${bulan}&tahun=${tahun}&unit_kerja=${unitKerja}&page=1&limit=10`
      );
      if (!tableResponse.ok) {
      ...
      const bigResponse = await fetch(
        `/api/admin/tcu-table-data?bulan=${bulan}&tahun=${tahun}&unit_kerja=${unitKerja}&page=1&limit=10000`
      );
  ...
  const fetchBopoData = async () => {
    try {
      ...
      const resp = await fetch(
        `/api/admin/bopo-tcu-dashboard?${params.toString()}`
      );
      if (!resp.ok) {
        setBopoData(null);
        return;
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="text-center max-w-lg">
          ...
          <p className="text-gray-600 mb-4">
            Silakan unggah data TCU terlebih dahulu atau pilih periode lain.
          </p>
    ...
    <>
      ...
      >
        ...
        <div className="pdf-region-marker hidden print:block print:relative print:bg-yellow-400 print:border-b-4 print:border-black print:z-50 print:py-3 print:px-4 print:text-center print:font-bold print:text-black print:text-2xl print:mb-4">
          ...
          {selectedDaerahInfo?.nama || "TCU Kantor Pusat"}
        </div>
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-6">
            <div>
              <h1 className="text-xl lg:text-2xl font-bold">
                DEMOGRAFI SDM TCU - {getExportTitle()}
              </h1>
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-4">
              <h2 className="text-lg font-bold text-center">
                KONSOLIDASI TCU - DEMOGRAFI PEKERJA - {selectedDaerahInfo?.kode}
              </h2>
            </div>
            <div className="p-6">
              {stats?.chartData && (
