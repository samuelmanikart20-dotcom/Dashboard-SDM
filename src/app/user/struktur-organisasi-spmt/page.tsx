...
export default function ViewStrukturOrganisasiSPMTPage() {
  ...
  const fetchAvailablePeriodsAndDaerah = async () => {
    ...
    try {
      const response = await fetch(
        "/api/admin/struktur-organisasi/available-periods",
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        ...
        console.error(
          `[ViewStrukturOrganisasi SPMT User] API error: ${response.status} - ${errorText}`
        );
      ...
      if (!result.success) {
        console.error(
          `[ViewStrukturOrganisasi SPMT User] API returned error:`,
          result.error
        );
      ...
      if (result.data) {
        ...
        console.log(
          `[ViewStrukturOrganisasi SPMT User] Received ${periods.length} periods and ${directorates.length} directorates from API`
        );
        ...
        if (periods.length > 0) {
          ...
          if (!selectedPeriod || selectedPeriod !== latestPeriod) {
            ...
            console.log(
              `[ViewStrukturOrganisasi SPMT User] Set default period to: ${latestPeriod}`
            );
          }
        }
        ...
        if (directorates.length > 0) {
          ...
          if (!selectedDirektorat || selectedDirektorat !== firstDirektorat) {
            ...
            console.log(
              `[ViewStrukturOrganisasi SPMT User] Set default directorate to: ${firstDirektorat}`
            );
          }
        }

        console.log(
          `[ViewStrukturOrganisasi SPMT User] Loaded ${periods.length} periods and ${directorates.length} directorates`
        );
      } else {
        console.warn(`[ViewStrukturOrganisasi SPMT User] No data in response`);
        setAvailablePeriods([]);
        setDirektoratList([]);
      }
    } catch (error: any) {
      ...
      console.error(
        `[ViewStrukturOrganisasi SPMT User] Error details:`,
        errorMessage
      );
      setAvailablePeriods([]);
      setDirektoratList([]);
  ...
  const handlePrint = () => {
    ...
    if (selectedDirektorat === "all") {
          ...
          orgChartsHTML += `
            <div class="print-daerah-section" style="page-break-after: always; margin-bottom: 40px;">
              <div class="print-header">
                <h1>STRUKTUR ORGANISASI SPMT - ${direktorat.label}</h1>
      ...
      const direktoratName =
        direktoratList.find((d) => d.value === selectedDirektorat)?.label ||
        "Direktorat";
      orgChartsHTML = `
        <div class="print-header">
          <h1>STRUKTUR ORGANISASI SPMT - ${direktoratName}</h1>
          <p>Periode: ${periodLabel}</p>
        </div>
        <div class="print-container">
    ...
    const printHTML = `
      ...
      <html>
        <head>
          <title>Struktur Organisasi SPMT - ${printTitle}</title>
          <style>
            ${styles}
            @page {
              size: auto;
            ...
            @media print {
              ...
              *::after {
              ...
              .react-flow__node > div p,
              .react-flow__node > div span {
                font-size: 0.9em !important;
              }
              .react-flow__node > div .text-base {
                font-size: 0.95em !important;
              }
              .react-flow__node > div .text-sm {
                font-size: 0.85em !important;
              }
              .react-flow__node > div .text-xs {
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-blue-900 mb-1 flex items-center gap-2">
                ...
                Lihat Struktur Organisasi SPMT
              </h1>
              <p className="text-gray-600 text-sm">
                Lihat bagan struktur organisasi berdasarkan periode dan
                direktorat yang dipilih (Mode View Only)
              </p>
            </div>
        ...
        {selectedDirektorat && selectedDirektorat !== "" && selectedPeriod ? (
          selectedDirektorat === "all" ? (
                ...
                <div key={direktorat.value} className="h-screen w-full">
                  <div className="bg-white px-4 py-2 border-b border-gray-200">
                    <h2 className="text-lg font-semibold text-gray-800">
                      Struktur Organisasi SPMT - {direktorat.label}
                    </h2>
            ...
            <div className="flex-1 flex flex-col h-[calc(100vh-100px)]">
              <div className="bg-white px-4 py-2 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800">
                  Struktur Organisasi SPMT -{" "}
                  {direktoratList.find((d) => d.value === selectedDirektorat)
                    ?.label || ""}
                </h2>
                <p className="text-sm text-gray-600">
                  Periode:{" "}
