  ...
  const fetchAvailablePeriodsAndDaerah = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[ViewStrukturOrganisasi IKT User] API error: ${response.status} - ${errorText}`);
      ...
      if (!result.success) {
        console.error(`[ViewStrukturOrganisasi IKT User] API returned error:`, result.error);
      ...
      if (result.data) {
        ...
        console.log(`[ViewStrukturOrganisasi IKT User] Received ${periods.length} periods and ${directorates.length} directorates from API`);
        ...
        if (periods.length > 0) {
          ...
          if (!selectedPeriod || selectedPeriod !== latestPeriod) {
            ...
            console.log(`[ViewStrukturOrganisasi IKT User] Set default period to: ${latestPeriod}`);
          }
        }
        
        console.log(`[ViewStrukturOrganisasi IKT User] Loaded ${periods.length} periods and ${directorates.length} directorates`);
      } else {
        console.warn(`[ViewStrukturOrganisasi IKT User] No data in response`);
        setAvailablePeriods([]);
        setDirektoratList([]);
      }
      } catch (error: any) {
        ...
        console.error(`[ViewStrukturOrganisasi IKT User] Error details:`, errorMessage);
        setAvailablePeriods([]);
        setDirektoratList([]);
      } finally {
        setLoading(false);
