  ...
  const fetchAvailablePeriodsAndDaerah = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[ViewStrukturOrganisasi TCU User] API error: ${response.status} - ${errorText}`);
      ...
      if (!result.success) {
        console.error(`[ViewStrukturOrganisasi TCU User] API returned error:`, result.error);
      ...
      if (result.data) {
        ...
        console.log(`[ViewStrukturOrganisasi TCU User] Received ${periods.length} periods and ${directorates.length} directorates from API`);
        ...
        if (periods.length > 0) {
          ...
          if (!selectedPeriod || selectedPeriod !== latestPeriod) {
            ...
            console.log(`[ViewStrukturOrganisasi TCU User] Set default period to: ${latestPeriod}`);
          }
        }
        
        console.log(`[ViewStrukturOrganisasi TCU User] Loaded ${periods.length} periods and ${directorates.length} directorates`);
      } else {
        console.warn(`[ViewStrukturOrganisasi TCU User] No data in response`);
        setAvailablePeriods([]);
        setDirektoratList([]);
      }
      } catch (error: any) {
        ...
        console.error(`[ViewStrukturOrganisasi TCU User] Error details:`, errorMessage);
        setAvailablePeriods([]);
        setDirektoratList([]);
      } finally {
        setLoading(false);
